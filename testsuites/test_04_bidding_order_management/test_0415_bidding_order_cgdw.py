...
@unittest.skip
class TestBiddingOrder0415(MyunitTest):
    """采购单位受理结果，生成待专家审核状态的竞价单"""

    def test_bidding_order_0415(self):
        ...
        time.sleep(80)
        # BasePage(self.driver).open(cgsw_bidding_list_url)  # 切换到采购单位账号
        BiddingOrderManagement(self.driver).confirm_result('受理结果通过', cgsw_bidding_list_url)
        flag = BiddingOrderManagement(self.driver).check_confirm_result_ghzx()  # 受理通过后，状态变为专家结果审核
        self.assertTrue(flag, '受理通过后，状态变未变为专家结果审核')
        if flag:
            BiddingOrderManagement(self.driver).get_img('test_0415_bidding_order_pass.png')
        else:
            BiddingOrderManagement(self.driver).get_img('test_0415_bidding_order_fail.png')
