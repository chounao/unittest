...
@unittest.skip
class TestBiddingOrder0417(MyunitTest):
    """采购单位申请二次议价"""

    def test_bidding_order_0417(self):
        ...
        flag = BiddingOrderManagement(self.driver).check_bargain_status_gys()
        self.assertTrue(flag, '专家审核通过后，状态变未变为竞价完成')
        if flag:
            BiddingOrderManagement(self.driver).get_img('test_0417_bidding_order_pass.png')
        else:
            BiddingOrderManagement(self.driver).get_img('test_0417_bidding_order_fail.png')
