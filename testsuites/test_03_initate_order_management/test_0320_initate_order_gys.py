...
@unittest.skip
class TestGysFinallyInitateOrder(MyunitTest):
    """国和中心审核通过直购单-供应商生成订购完成状态的直购单"""

    def test_gys_finally_initate_order(self):
        ...
        OrderManagement(self.driver).add_initateorder(1, 1, '柏柏测试订购单')  # 生成待审核状态的订购单
        ...
        time.sleep(5)
        LoginPage(self.driver).again_login(gys_username, gys_password, gys_initate_order_url)  # 切换到供应商账号
        flag = OrderManagement(self.driver).check_status_gys()
        self.assertTrue(flag, '国和中心审核通过后，供应商未生成订状态的订购单')
        if flag:
            OrderManagement(self.driver).get_img('test_0320_gys_finally_initate_order_pass.png')
        else:
            OrderManagement(self.driver).get_img('test_0320_gys_finally_initate_order_fail.png')
        time.sleep(5)
