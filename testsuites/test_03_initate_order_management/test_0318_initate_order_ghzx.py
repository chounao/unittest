...
@unittest.skip
class TestGhzxCheckInitateOrder(MyunitTest):
    """国和中心审核通过直购单-生成订购完成状态订购单"""

    def test_ghzx_examine_initate_order(self):
        ...
        flag = OrderManagement(self.driver).check_examined_status_ghzx()
        self.assertTrue(flag, '国和中心审核通过后，未生成订购完成状态的订购单')
        if flag:
            OrderManagement(self.driver).get_img('test_0318_ghzx_examine_initate_order_pass.png')
        else:
            OrderManagement(self.driver).get_img('test_0318_ghzx_examine_initate_order_fail.png')
        time.sleep(5)
