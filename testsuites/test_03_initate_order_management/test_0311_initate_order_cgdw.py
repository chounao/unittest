...
@unittest.skip
class TestInitateOrder0311(MyunitTest):
    """发起订购"""

    def test_initate_order_0311(self):
        ...
        flag = OrderManagement(self.driver).check_initateorder()  # 检查是否生成待审核状态的订购单
        self.assertTrue(flag, '订购单提交失败')
        if flag:
            OrderManagement(self.driver).get_img('test_0311_initate_order_pass.png')
        else:
            OrderManagement(self.driver).get_img('test_0311_initate_order_fail.png')
