...
class TestGhzxCheckInitateOrder(MyunitTest):
    ...
    def test_ghzx_check_initate_order(self):
        ...
        flag = OrderManagement(self.driver).check_examine_ghzx()
        self.assertTrue(flag, '国和中心未生成待专家结果审核状态额订购单')
        if flag:
            OrderManagement(self.driver).get_img('test_0317_ghzx_check_initate_order_pass.png')
        else:
            OrderManagement(self.driver).get_img('test_0317_ghzx_check_initate_order_fail.png')
        time.sleep(5)
