...
@unittest.skip
class TestInitateOrder0313(MyunitTest):
    """订购单-采购单位审核通过"""

    def test_initate_order_0313(self):
        ...
        OrderManagement(self.driver).cgdw_check('同意')  # 采购单位审核通过
        OrderManagement(self.driver).cgdw_remove('订购单信息错误，申请撤回')  # 采购单位撤回订购单
        flag = OrderManagement(self.driver).check_remove()  # 检查是否生成待审核状态的订购单
        self.assertTrue(flag, '采购单位撤回订购单失败')
        if flag:
            OrderManagement(self.driver).get_img('test_0313_initate_order_pass.png')
        else:
            OrderManagement(self.driver).get_img('test_0313_initate_order_fail.png')
