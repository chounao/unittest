...
class TestCgdwReviceGoods(MyunitTest):
    ...
    def test_cgdw_revice_goods(self):
        ...
        # 采购单位验收
        OrderPage(self.driver).cgdw_acceptance()
        # 检查是否验收成功
        flag = OrderPage(self.driver).check_acceptance()
        if flag:
            self.assertTrue(flag)
            OrderPage(self.driver).get_img('test_33317_pass.png')
            log.logger.info('运行成功：验收成功')
        else:
            self.assertFalse(flag)
            OrderPage(self.driver).get_img('test_33317_fail.png')
            log.logger.info('运行失败：验收失败')
