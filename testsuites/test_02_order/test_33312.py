...
@unittest.skip
class TestCancelOrder(MyunitTest):
    """校验供应商未接单，采购单位取消订单"""
    ...
    def test_cancel_order(self):
        ...
        # 取消订单
        OrderPage(self.driver).cancel_order()
        # 检查取消订单否提交成功
        flag = OrderPage(self.driver).check_cancel_order()
        if flag:
            self.assertTrue(flag)
            OrderPage(self.driver).get_img('test_33312_pass.png')
            log.logger.info('运行成功：取消订单成功')
        else:
            self.assertFalse(flag)
            OrderPage(self.driver).get_img('test_33312_fail.png')
            log.logger.info('运行失败：取消订单失败')
